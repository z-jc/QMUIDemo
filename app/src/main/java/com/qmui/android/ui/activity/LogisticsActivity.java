...
import com.qmui.android.okgomvp.entity.CourierEntity;
import com.qmui.android.okgomvp.http.JsonUtil;
import com.qmui.android.okgomvp.http.OkgoConfig;
import com.qmui.android.okgomvp.model.ApiModel;
import com.qmui.android.okgomvp.model.ApiModelImpl;
import com.qmui.android.okgomvp.model.OnApiListener;
...
public class LogisticsActivity extends BaseActivity implements View.OnClickListener {
    ...
    private String COM_TYPE = "";       //快递公司编号
    ...
    public void initView() {
        ...
        map.put("key", OkgoConfig.APP_KEY);
        apiModel.getCourier(map, new OnApiListener() {
            @Override
            public void onStart() {
            ...
            public void onSuccess(String response) {
                courierEntity = JsonUtil.fromJson(response, new CourierEntity());
                if (courierEntity.getResultcode().equals(courierEntity.result_ok)) {
                    List<String> list = new ArrayList<>();
                    for (CourierEntity.ResultBean resultBean : courierEntity.getResult()) {
                        list.add(resultBean.getCom());
                    }
                    setSpinnerText(list);
                }
            }
     ...
     * @param list
     */
    private void setSpinnerText(List<String> list) {
        spinner.setItems(list);
        spinner.setOnItemSelectedListener(new MaterialSpinner.OnItemSelectedListener<String>() {
            @Override
            public void onItemSelected(MaterialSpinner view, int position, long id, String item) {
                Snackbar.make(view,
                        courierEntity.getResult().get(position).getCom()
                                + "->"
                                + courierEntity.getResult().get(position).getNo(),
    ...
    private void start(int code) {
        int scan_type = QrConfig.TYPE_BARCODE;          //扫描条形码
        int scan_view_type = QrConfig.SCANVIEW_TYPE_BARCODE;        //条形码框
        QrConfig qrConfig = new QrConfig.Builder()
                .setDesText("扫描条码")//扫描框下文字
                ...
                .create();
        QrManager.getInstance().init(qrConfig).startScan(LogisticsActivity.this, new QrManager.OnScanResultCallback() {
            @SuppressLint("MissingPermission")
            @Override
            public void onScanSuccess(int code, String result) {
                Log.e(TAG, "result:" + result + ",code:" + code);
                ToastUtil.showShortToast(LogisticsActivity.this, "result:" + result);
                vibrator.vibrate(30);
                switch (code) {
